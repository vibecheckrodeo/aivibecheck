...
export function emailConfiguration(env) {
  const transactional = Boolean(env.RESEND_API_KEY && env.EMAIL_FROM && /^[a-f0-9]{64}$/i.test(env.INTEGRATION_ENCRYPTION_KEY || ''));
  return { transactional, marketing: transactional && Boolean(env.EMAIL_POSTAL_ADDRESS), provider: 'resend' };
}
export async function initializeContact(env, row, consent = false) {
  await env.DB.prepare('INSERT OR IGNORE INTO email_contacts(request_id,consent_at) VALUES(?,?)').bind(row.id, consent === true ? Date.now() : null).run();
}
export async function emailStatus(env, id) {
  ...
  return { ...emailConfiguration(env), consent: Boolean(c?.consent_at && !c?.unsubscribed_at), verified: Boolean(c?.verified_at), messages, materialRequests };
}
export async function queueEmail(env, row, { kind, subject, message, category = 'transactional', dueAt = Date.now(), sequence = null }) {
  ...
  await env.DB.prepare('INSERT OR IGNORE INTO email_outbox(id,request_id,category,kind,sequence,subject,message,created_at,due_at) VALUES(?,?,?,?,?,?,?,?,?)')
    .bind(crypto.randomUUID(), row.id, category, kind, sequence, subject, message, Date.now(), dueAt).run();
}
export async function saveReply(env, row, data) {
  if (['declined','expired'].includes(row.status)) fail(409, 'This request is closed.');
  ...
  const now = Date.now();
  await env.DB.prepare('UPDATE requests SET reply=?,estimated_minutes=?,reply_updated_at=?,upsell_enabled=?,upsell_count=?,updated_at=? WHERE id=?')
    .bind(reply, data.estimatedMinutes, now, Number(data.upsellEnabled), data.upsellCount, now, row.id).run();
...
export async function requestMaterials(env, row, message) {
  if (!['draft','submitted'].includes(row.status)) fail(409, 'Material requests are available while the project is being shared.');
...
export async function resendVerification(env, row, now = Date.now()) {
...
export async function handleCommunications(request, env, path, method, helpers, available) {
  ...
  if (path[0]==='requests' && path[1] && ['email','verify-email'].includes(path[2])) {
    ...
    if (method==='POST' && path[2]==='verify-email') { await resendVerification(env,row); return json({queued:true}); }
  }
...
export const dripCopy = [
  ...
  ['One last note before your review', 'This is my last note about booking more time. If you want a longer session, check the available times on your request page. Otherwise, we’ll use the 15 minutes for an answer or a conversation about your project.']
];
async function eligible(env, row, c, available, now) {
  if (!row || !c?.verified_at || !c.consent_at || c.unsubscribed_at || !row.upsell_enabled || !row.reply || !row.paid_at || !['paid','booked'].includes(row.status)) return false;
  ...
  return (await available(env,row,row.estimated_minutes,now)).length > 0;
}
async function scheduleDrips(env, available, now) {
  const rows = (await env.DB.prepare("SELECT * FROM requests WHERE upsell_enabled=1 AND paid_at IS NOT NULL AND status IN ('paid','booked')").all()).results;
...
async function scheduleReceipts(env) {
  const rows=(await env.DB.prepare("SELECT r.* FROM requests r JOIN email_contacts c ON c.request_id=r.id WHERE r.status IN ('approved','paid','booked') ORDER BY r.updated_at DESC LIMIT 200").all()).results;
  for(const row of rows){
    if(row.status==='approved') await queueEmail(env,row,{kind:'approved',subject:'Your Vibe Check request is approved',message:`I’ve approved your request.\n\nReview focus: ${row.scope}\n\nOpen your private request page to see my reply, pay the $25 deposit when checkout is available, and choose a time. The deposit covers the first 15 minutes.\n\nAshley`});
    if(row.paid_at) await queueEmail(env,row,{kind:'deposit-confirmed',subject:'Your Vibe Check deposit is confirmed',message:'Your $25 deposit has been verified. It covers an answer or a 15-minute conversation. If you haven’t booked yet, choose an available appointment on your request page.'});
    if(row.booked_slot_id){
...
export async function processEmails(env, available = async()=>[], now = Date.now()) {
  ...
  const contacts=(await env.DB.prepare('SELECT r.* FROM requests r JOIN email_contacts c ON c.request_id=r.id WHERE c.verified_at IS NULL AND r.status NOT IN (\'expired\',\'declined\') AND NOT EXISTS(SELECT 1 FROM email_outbox o WHERE o.request_id=r.id AND o.category=\'verification\') LIMIT 20').all()).results;
  for (const row of contacts) await resendVerification(env,row,now);
  ...
  // A successful send API response is acceptance, not evidence of inbox delivery.
  const pending=(await env.DB.prepare("SELECT * FROM email_outbox WHERE state IN ('accepted','delivered') AND accepted_at>? AND (checked_at IS NULL OR checked_at<?) ORDER BY checked_at LIMIT 25").bind(now-7*DAY,now-3600000).all()).results;
  for (const job of pending) {
    try {
      const response=await (env.EMAIL_FETCH || fetch)(`https://api.resend.com/emails/${encodeURIComponent(job.provider_id)}`,{headers:{Authorization:`Bearer ${env.RESEND_API_KEY}`},signal:AbortSignal.timeout(20000)});
      if (!response.ok) continue;
      const value=await response.json(); if (value.id!==job.provider_id) continue;
      const state=['delivered','bounced','complained'].includes(value.last_event)?value.last_event:job.state;
      await env.DB.prepare('UPDATE email_outbox SET state=?,checked_at=?,delivered_at=CASE WHEN ?=\'delivered\' THEN COALESCE(delivered_at,?) ELSE delivered_at END WHERE id=?').bind(state,now,state,now,job.id).run();
      if (['bounced','complained'].includes(state)) await suppressMarketing(env,await get(env,job.request_id),state);
    } catch { /* Keep acceptance separate from delivery, and retry the receipt check. */ }
  }
