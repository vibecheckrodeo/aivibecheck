...
export async function availableReviewSlots(env,row,minutes=15,now=Date.now()){
  ...
  for(const start of all){
    ...
    if(blocks.every(slot=>slot&&slot.ends_at===slot.starts_at+BLOCK&&slot.zoom_url===start.zoom_url&&(!slot.request_id||slot.request_id===row.id)&&(!slot.claimed_by||slot.claimed_by===row.id)))result.push({id:start.id,starts_at:start.starts_at,ends_at:start.starts_at+minutes*60000});
  }
  return result;
}
...
export async function reconcilePendingReviews(env,now=Date.now()){
  const attempts=(await env.DB.prepare("SELECT * FROM review_payments WHERE status IN ('creating','open') ORDER BY created_at LIMIT 100").all()).results;
...
export async function bookReview(env,row,slotId,now=Date.now()){
  ...
  let result;try{result=await env.DB.batch([
    env.DB.prepare("UPDATE slots SET request_id=? WHERE id=? AND request_id IS NULL AND EXISTS(SELECT 1 FROM requests WHERE id=? AND paid_at IS NOT NULL AND booked_slot_id IS NULL AND NOT EXISTS(SELECT 1 FROM review_payments p WHERE p.request_id=requests.id AND p.status IN ('creating','open')))").bind(row.id,slotId,row.id),
    env.DB.prepare("UPDATE requests SET booked_slot_id=?,review_minutes=15,status='booked',updated_at=? WHERE id=? AND booked_slot_id IS NULL AND EXISTS(SELECT 1 FROM slots WHERE id=? AND request_id=?)").bind(slotId,now,row.id,slotId,row.id)
...
export async function createReviewCheckout(env,row,slotId,minutes,origin,now=Date.now()){
  ...
  const payload={mode:'payment','payment_method_types[0]':'card',customer_email:row.email,
