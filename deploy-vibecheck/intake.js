...
$('ashley-reply').hidden=!row.reply;$('reply-text').textContent=row.reply||'';$('time-estimate').textContent=row.estimated_minutes?`My estimate: ${row.estimated_minutes} minutes.`:'';if(! $('scheduling').hidden){configureDuration();await loadSlots();}if(row.booking){$('booking-time').textContent=`${time(row.booking.starts_at)} · ${(row.booking.ends_at-row.booking.starts_at)/60000} minutes`;$('zoom').href=row.booking.zoom_url;}await Promise.all([loadConnections(),loadEmailStatus()]);}
...
$('book-slot').addEventListener('click',()=>busy($('book-slot'),async()=>{
  ...
  else {await render(await api(`requests/${id}/book`,'POST',{slotId:chosenSlot}));notice('Your appointment is booked. Your Zoom link is below.');}
}));
